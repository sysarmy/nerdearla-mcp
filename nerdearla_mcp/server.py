...
async def get_speakers(
    event_id: Annotated[
        str | None, "Event ID to get speakers for (defaults to nearest event)"
    ] = None,
    q: Annotated[
        str | None,
        "Optional search query to filter speakers by name, bio, and tagline (case-insensitive)",
    ] = None,
    ...
    try:
        ...
        if only_top_speakers:
            speakers = [
                speaker for speaker in speakers if speaker.get("isTopSpeaker", False)
            ]
...
async def get_speaker_details(
    ...
    event_id: Annotated[
        str | None, "Event ID to get speaker details for (defaults to nearest event)"
    ] = None,
) -> Dict[str, Any]:
    """
...
async def get_sessions(
    event_id: Annotated[
        str | None, "Event ID to get sessions for (defaults to nearest event)"
    ] = None,
    q: Annotated[
        str | None,
        "Optional search query to filter sessions by title, description, room, and speaker names (case-insensitive)",
    ] = None,
    only_plenum: Annotated[
        bool, "Whether to return only plenum sessions (keynotes)"
    ] = False,
    start_time: Annotated[
        str | None,
        "Optional start timestamp filter (ISO format with timezone, e.g., '2024-04-11T09:00:00-03:00' or '2024-04-11T09:00:00Z')",
    ] = None,
    end_time: Annotated[
        str | None,
        "Optional end timestamp filter (ISO format with timezone, e.g., '2024-04-11T12:00:00-03:00' or '2024-04-11T12:00:00Z')",
    ] = None,
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if only_plenum:
            sessions = [
                session for session in sessions if session.get("isPlenumSession", False)
            ]
            ...
            for session in sessions:
                ...
                if session_start:
                    try:
                        ...
                        if end_time:
                            end_filter = datetime.fromisoformat(
                                end_time.replace("Z", "+00:00")
                            )
                            if session_end:
                                session_end_datetime = datetime.fromisoformat(
        ...
        if q and q.lower() != "null":
            ...
            if q:
                ...
                for session in sessions:
                    ...
                    question_answers = session.get("questionAnswers", [])
                    for qa in question_answers:
                        answer = qa.get("answer", "")
                        if answer:  # Only add non-empty answers
                            searchable_fields.append(answer)

                    # Search in category items
                    categories = session.get("categories", [])
                    for category in categories:
                        category_items = category.get("categoryItems", [])
                        for item in category_items:
                            item_name = item.get("name", "")
                            if item_name:  # Only add non-empty category item names
                                searchable_fields.append(item_name)

                    # Check if query matches any field (skip None/null values)
                    if any(
                        query_pattern.search(str(field))
        ...
        return [
            {
                ...
                "speakers": [
                    speaker.get("name") for speaker in session.get("speakers", [])
                ],
...
async def get_session_details(
    ...
    event_id: Annotated[
        str | None, "Event ID to get session details for (defaults to nearest event)"
    ] = None,
) -> Dict[str, Any]:
    """
